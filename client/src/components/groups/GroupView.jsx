...
export default function GroupView() {
  ...
  const joinGroup = async (e) => {
    ...
    await axios
      .put("/api/groups/join", currentGroupInfo, headers)
      .then((res) => {
  ...
  const leaveGroup = async (e) => {
    ...
    await axios
      .post("/api/groups/leave", currentGroupInfo, headers)
      .then((res) => {
        ...
        <div class="flex-auto w-full md:w-3/4 lg:w-4/5 lg:p-5">
          <div className="flex lg:gap-8">
            <div class="w-full lg:w-3/4 bg-white relative lg:rounded-t-xl">
                      ...
                      <form>
                        <button
                          ...
                          class={`py-2.5 px-5 mr-2 mb-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 ${
                            group.status == false ? "cursor-not-allowed" : ""
                          }`}
                          disabled={group.status == false ? true : false}
                        >
                          {t("groups.join")}
                        </button>
                      </form>
              ...
              <div className="p-5">{group.description}</div>
            </div>
          </div>
        </div>
      </div>
