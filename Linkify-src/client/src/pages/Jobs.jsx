...
const Jobs = () => {
  ...
  const handleEditApplication = async (jobId, userId) => {
    ...
    const response = await fetch(
      `/api/user/jobPosts/updateApplication/${jobId}`,
      {
        method: "PUT",
        headers: {
  ...
  const recruiterJobs = jobs.filter((job) =>
    job.postedBy.includes(recruiterId)
  );
  ...
  const handleAddJobListing = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "../api/user/jobPosts/createJobPost",
        newJob
      );
  ...
  const handleDeleteJob = async (jobId) => {
    ...
    if (isConfirmed) {
      try {
        const response = await fetch(
          `/api/user/jobPosts/deleteJobPost?id=${jobId}`,
          {
            method: "DELETE",
          }
        );
  ...
  const handleRejectApplicant = async (event, jobId, applicantUserId) => {
    ...
    let isConfirmed;
    if (user.isRecruiter) {
      isConfirmed = window.confirm(t("jobs.rejectApplication"));
    } else {
      isConfirmed = window.confirm(t("jobs.withdrawApplication"));
    }

    if (isConfirmed) {
      try {
        const response = await fetch(
          `/api/user/jobPosts/rejectApplicant/${jobId}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
                      ...
                      <form>
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <button
                            ...
                            className="mt-3 w-2/3 cursor-pointer p-2 text-white font-semibold duration-150 transform border border-white rounded-lg hover:bg-blue-800 bg-indigo-400 group-hover:border-transparent"
                          >
                            {showAppliedJobs
                              ? t("jobs.available")
                              : t("jobs.applied")}
                  ...
                  <div>
                    {jobs
                      ...
                      .reduce((acc, job) => {
                        ...
                        const shouldDisplayJob = showAppliedJobs
                          ? searchTermFilter && userApplied
                          : searchTermFilter && !userApplied;
                      ...
                      .map(({ job, userApplied }) => (
                        <div
                          key={job._id}
                          className=" items-center grid gap-5 my-2 md:grid-cols-2 lg:grid-cols-1 rounded-xl shadow dark:bg-gray-800 dark:border-gray-700 text-black max-w-screen-xl"
                        >
                          <div
                                ...
                                <>
                                  {!userApplied ? (
                                    <div className="pt-5 text-right">
                                      <a
                                        onClick={(event) =>
                                          handleOpenJobApplicationModal(
                                            event,
                                            job._id
                                          )
                                        }
                                        className="cursor-pointer px-6 py-2 text-white font-semibold duration-150 transform border border-white rounded-full hover:bg-blue-800 bg-indigo-400 group-hover:border-transparent"
                                      >
                                        {t("jobs.apply")}
                                      </a>
                                    </div>
                                    ...
                                    <div className="pt-5 text-right">
                                      <button
                                        ...
                                        className="mr-4 text-red-600 underline"
                                      >
                                        {t("jobs.delete")}
                                      </button>
                                      <a
                                        onClick={(event) =>
                                          handleOpenEditApplicationModal(event)
                                        }
                                        className="cursor-pointer px-6 py-2 text-white font-semibold duration-150 transform border border-white rounded-full hover:bg-blue-800 bg-indigo-400 group-hover:border-transparent"
                                      >
                                        {t("jobs.edit")}
                            ...
                            <>
                              ...
                              <div className="fixed inset-0 flex items-center justify-center z-10">
                                ...
                                <div className="flex flex-col lg:h-3/4 lg:w-1/2 sm:w- bg-white max-w-lg lg:rounded-lg relative h-full ">
                                  <div
                                    className={
                                      "flex flex-col h-full pr-6 pl-6 pt-6 pb-16 justify-between"
                                    }
                                  >
                                    <label
                                    ...
                                    <div className="flex flex-col overflow-auto">
                                      <p className="text-left mt-8 text-2xl italic font-semibold text-lg">
                                        {t("jobs.desc")}:
                                      </p>
                                      ...
                                      {!userApplied ? (
                                        <div className="flex items-center justify-center w-full py-5">
                                          <a
                                            ...
                                            className="cursor-pointer m-auto text-center w-40 items-center justify-center pt-4 pb-4 font-medium text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                                          >
                                            {t("jobs.apply")}
                                          </a>
                                        </div>
                                        ...
                                        <div className="flex items-center justify-center w-full py-5">
                                          <a
                                            onClick={(event) =>
                                              handleOpenEditApplicationModal(
                                                event
                                              )
                                            }
                                            className="cursor-pointer px-6 py-2 text-white font-semibold duration-150 transform border border-white rounded-full hover:bg-blue-800 bg-indigo-400 group-hover:border-transparent"
                              ...
                              <div className="fixed inset-0 flex items-center justify-center z-10">
                                ...
                                <div className="bg-white w-full sm:max-w-md lg:max-w-lg p-6 my-8 mx-auto rounded-lg shadow-md z-10">
                                  <div className="flex justify-between items-center">
                                    ...
                                    <button
                                      onClick={handleCloseJobApplicationModal}
                            ...
                            <>
                              ...
                              <div className="fixed inset-0 flex items-center justify-center z-10">
                                ...
                                <div className="bg-white w-full sm:max-w-md lg:max-w-lg p-6 my-8 mx-auto rounded-lg shadow-md z-10">
                                  <div className="flex justify-between items-center">
                                    <h2 className="text-2xl font-bold">
                                      {t("jobs.applyJobTitle")}
                                    </h2>
                                    <button
                                      onClick={handleCloseEditApplicationModal}
                                      className="btn btn-sm btn-circle"
                                    >
                                      ✕
                                    </button>
                                  ...
                                  >
                                    ...
                                    <button
                                      ...
                                      className="mt-4 inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                                      onClick={() =>
                                        handleEditApplication(job._id, user._id)
                                      }
                                    >
                                      {t("jobs.update")}
                                    </button>
