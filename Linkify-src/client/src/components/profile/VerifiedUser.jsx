...
export default function UserIm(props) {
  ...
  const matchingTexts = verifiedUser.filter((user) => {
    ...
    const matchingWords = currentUserWords.filter((word) =>
  ...
  return (
    <div
      className={`w-auto verifiedUser text-[1rem] pl-1 ${
        type == "dms" ? "" : "tooltip tooltip-right"
      }`}
      data-tip="Verified"
    >
      {matchingTexts.length > 0 && <MdVerified />}
    </div>
  );
}
