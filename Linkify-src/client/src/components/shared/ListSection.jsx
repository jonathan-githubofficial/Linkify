...
function ListSection(props) {
  ...
  if (props.type == "jobs") {
    ...
    type_redirect_url = "/";
    type_submit_button = t("listSection.apply");
  } else if (props.type == "events") {
    type_host = location;
    var eventDate = new Date(event_date);
    var eventDateFormatted = moment(eventDate)
      .utc()
    ...
    type_submit_button = t("listSection.viewMore");
  } else if (props.type == "groups") {
        ...
        <div class="flex items-center">
          ...
          <div class="w-1/3 flex justify-end">
            {eventDaysDifference > 0 && (
              <div className="badge badge-error gap-2 mb-4">Event Expired</div>
            )}
          </div>
        </div>
        <div>
          ...
          <span
            className={
              "jobs-location " + (props.type == "events" ? "hidden" : "")
            }
          >
            {location}
          </span>

          <div className="jobs-description pt-5">{trimmedDescription}</div>
        </div>
